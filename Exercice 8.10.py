...
class runge_kutta:
    ...
    def simulation(self):
        ...
        def rho(i, j):
            ...
            return 30 * self.h * 1000 /np.sqrt(eps_x**2 + eps_y**2 + eps_z**2)
        
        def h_prime(rho):
            return self.h * rho**0.25
        ...
        for t in tqdm(self.tpoints):
            ...
            if p > 1:
                k1, k2, k3, k4 = self.runge_kutta(self.h, self.cond_init)
                self.cond_init += (k1+2*k2+2*k3+k4)/6
                self.h = h_prime(p)
                if self.h >=2*self.og_h:
                    self.h = 2*self.og_h
                # else:
                #     self.h =1.0001*self.h
...
if __name__ == "__main__":
    t, x, vx, y, vy, z, vz, h = runge_kutta(0, 2e9, 100000, [4e12, 0, 0, 500, 0, 0]).simulation()
